...
var Environment = {
	 ...
	 * @param {object}
	 *            t The sub-object.
	 * @returns {ServerRuntime} t The sub-object.
	 */
	load : function(t) {
		if (this.__env !== undefined || this.__env !== null) {
			return this.__env;
		}
		return t;
	}
...
function __buildBaseTypes(srt) {
	...
	/**
	 * @class
	 * @alias ClassLoadException
	 */
	srt.ClassLoadException = srt.Exception.extend( /** @lends ClassLoadException */
	{
		/** @member */
		init : function(message) {
...
function ServerRuntime(config, prefs) {
	...
	this.ClassBuilder = function(basetype, baseproto) {
		...
		/**
		 * @global
		 * @class {Class} The system class type
		 */
		var Class = function() {
		}
		...
		Class.prototype.byName = __basetype.byname || function(name) {
			...
			return env.ClassDict.ldefClass(name);
		}
		 ...
		 */
		/**
		 * Extend a class with a prototype and an optional classname.
		 * 
		 * @alias Class.extend
		 * @returns Function
		 * @member
						...
						if (!__initializing && this.init) { // doing init?
							try {
								...
								if (env === undefined || env === null) {
									e1 = new Error("Unable to locate Environment.");
									console.error("Classful machine error.", e1);
									throw new Error("CLASS_CFR: Exception in VM.", e1);
								}

								try { // find cle
									cle = env.ClassDict.ldefClass("ErrorInClsInitException");
								} catch (e) { /* void */
								}
								if (cle === null) { // no cle?
									cle = env.ClassDict.ldefClass("ClassLoadException");
								...
								if (e instanceof env.Exception) { // got ex?
									try { // wrap it
